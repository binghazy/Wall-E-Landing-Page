...
const words = ["organize", "search", "publish", "recover"];
...
function BlurWord({ word, trigger }: { word: string; trigger: number }) {
  ...
  // pink glow colours cycling across letter positions (darker, richer pinks)
  const gradientColors = ["#ff9ecf", "#ff66b3", "#ff59a6", "#ff3a9d"];
  ...
  return (
    <>
      {letters.map((char, i) => {
        ...
        const glow = `0 0 10px rgba(${r},${g},${b},0.95), 0 0 30px rgba(${r},${g},${b},0.5)`;
        return (
          <span
            key={i}
            style={{
              ...
              textShadow: showGradient ? glow : "none",
    ...
    <section className="relative min-h-screen flex flex-col justify-center items-start overflow-hidden bg-black">
        ...
        <div className="lg:max-w-[55%]">
          ...
          >
            <span className="inline-flex items-center gap-3 text-sm font-mono text-white/60">
              ...
              AI-powered document workspace with real-time sync
            </span>
          </div>
          ...
          <div className="mb-12">
            <h1
              className={`text-left text-[clamp(2rem,6vw,7rem)] font-display font-black leading-[0.92] tracking-tight text-white transition-all duration-1000 ${
                isVisible
                  ? "opacity-100 translate-y-0"
      ...
      >
        <div className="max-w-[1400px] mx-auto flex items-start gap-10 lg:gap-20">
          {[
            { value: "∞", label: "nesting levels" },
            { value: "100%", label: "live sync" },
